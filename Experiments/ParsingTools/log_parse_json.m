...
for i = 1:numel(tlines)
    ...
    if i == 1
        init_time = row.timestamp/1000;
        out.time = [out.time 0];
    else
        out.time = [out.time row.timestamp/1000-init_time];
    end
    ...
    out.x680nm = [out.x680nm row.readings(8)/(2^(row.gain-1) *0.00278*(row.atime+1)*(row.astep+1))];
    out.Clear = [out.Clear row.readings(9)/(2^(row.gain-1) *0.00278*(row.atime+1)*(row.astep+1))]; 
    out.NIR = [out.NIR row.readings(10)/(2^(row.gain-1) *0.00278*(row.atime+1)*(row.astep+1))];     
